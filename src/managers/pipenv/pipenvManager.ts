...
export class PipenvManager implements EnvironmentManager, Disposable {
    ...
    async refresh(context: RefreshEnvironmentsScope): Promise<void> {
        if (context === undefined) {
            await withProgress(
                {
                    location: ProgressLocation.Window,
                    title: PipenvStrings.pipenvRefreshingEnvs,
                },
                async () => {
    ...
    private async loadEnvMap() {
        ...
        // If a global environment is still not set, try using the latest available
        if (!this.globalEnv) {
            this.globalEnv = getLatest(this.collection.filter((env) => env.group === PIPENV_GLOBAL));
        }

        // Find any pipenv environments that might be associated with the current projects
        // These are environments whose project directories match workspace folders
        const pathSorted = this.collection
            .filter((e) => e.group && e.group !== PIPENV_GLOBAL)
            .sort((a, b) => {
        ...
        for (const p of paths) {
            ...
            if (env) {
                ...
                // then see if there is one in the collection for this project
                const found = pathSorted.find((e) => {
                    // Check if the environment's group matches the project directory name
                    return e.group && typeof e.group === 'string' && e.group === path.basename(p);
                });
                if (found) {
                    this.fsPathToEnv.set(p, found);
                }
