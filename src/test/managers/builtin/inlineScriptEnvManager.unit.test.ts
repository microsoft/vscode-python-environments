...
suite('InlineScriptEnvManager', () => {
    ...
    setup(async () => {
        ...
        baseExecutable = path.join(tempRoot, 'base-python', process.platform === 'win32' ? 'python.exe' : 'python');
    ...
    suite('cache reuse', () => {
        ...
        test('does not inspect or modify an entry resolving outside the physical cache root', async function () {
            ...
            await fs.outputFile(markerPath, 'keep');
            try {
                await fs.symlink(externalEnv, envDir().fsPath, process.platform === 'win32' ? 'junction' : 'dir');
            } catch (error) {
        ...
        test('does not inspect or modify an entry aliasing another hash directory', async function () {
            ...
            await fs.outputFile(markerPath, 'keep');
            try {
                await fs.symlink(otherEnv, envDir().fsPath, process.platform === 'win32' ? 'junction' : 'dir');
            } catch (error) {
                const code = (error as NodeJS.ErrnoException).code;
                if (code === 'EPERM' || code === 'EACCES') {
                    this.skip();
